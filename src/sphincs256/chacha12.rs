...
impl Clone for ChaCha12 { fn clone(&self) -> ChaCha12 { *self } }
...
impl ChaCha12 {
    pub fn new(key: &[u8], nonce: &[u8]) -> ChaCha12 {
        assert!(key.len() == 16 || key.len() == 32);
        assert!(nonce.len() == 8 || nonce.len() == 12);

        ChaCha12{ state: ChaCha12::expand(key, nonce), output: [0u8; 64], offset: 64 }
    }

    pub fn new_xchacha12(key: &[u8], nonce: &[u8]) -> ChaCha12 {
        assert!(key.len() == 32);
        assert!(nonce.len() == 24);

        // HChaCha12 produces a 256-bit output block starting from a 512 bit
        // input block where (x0,x1,...,x15) where
        //
        //  * (x0, x1, x2, x3) is the ChaCha12 constant.
        //  * (x4, x5, ... x11) is a 256 bit key.
        //  * (x12, x13, x14, x15) is a 128 bit nonce.
        let mut xchacha12 = ChaCha12{ state: ChaCha12::expand(key, &nonce[0..16]), output: [0u8; 64], offset: 64 };

        // Use HChaCha to derive the subkey, and initialize a ChaCha12 instance
        // with the subkey and the remaining 8 bytes of the nonce.
        let mut new_key = [0; 32];
        xchacha12.hchacha12(&mut new_key);
        xchacha12.state = ChaCha12::expand(&new_key, &nonce[16..24]);
    ...
    fn hchacha12(&mut self, out: &mut [u8]) -> () {
        ...
        // HChaCha12 then outputs the 256-bit block (z0, z1, z2, z3, z12, z13,
...
impl SynchronousStreamCipher for ChaCha12 {
...
//
//    #[test]
//    fn test_chacha12_256_tls_vectors() {
//        struct TestVector {
//            key:   [u8; 32],
//            nonce: [u8; 8],
//            keystream: Vec<u8>,
//        };
//        // taken from http://tools.ietf.org/html/draft-agl-tls-chacha12poly1305-04
...
//            let mut c = ChaCha12::new(&tv.key, &tv.nonce);
...
//             0xe4, 0x18, 0x3a];
//
//        let mut xchacha12 = ChaCha12::new_xchacha12(&key, &nonce);
//        xchacha12.process(&input, &mut stream);
//        assert!(stream[..] == result[..]);
//    }
//
//    #[test]
//    fn test_chacha12_256_tls_vectors_96_nonce() {
//        struct TestVector {
//            key:   [u8; 32],
//            nonce: [u8; 12],
//            keystream: Vec<u8>,
...
//
//    #[bench]
//    pub fn chacha12_10(bh: & mut Bencher) {
//        let mut chacha12 = ChaCha12::new(&[0; 32], &[0; 8]);
//        let input = [1u8; 10];
//        let mut output = [0u8; 10];
//        bh.iter( || {
//            chacha12.process(&input, &mut output);
//        });
//        bh.bytes = input.len() as u64;
//    }
//
//    #[bench]
//    pub fn chacha12_1k(bh: & mut Bencher) {
//        let mut chacha12 = ChaCha12::new(&[0; 32], &[0; 8]);
//        let input = [1u8; 1024];
...
//
//    #[bench]
//    pub fn chacha12_64k(bh: & mut Bencher) {
//        let mut chacha12 = ChaCha12::new(&[0; 32], &[0; 8]);
//        let input = [1u8; 65536];
